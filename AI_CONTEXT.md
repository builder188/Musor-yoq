...
## 2026-07-17 (2) KO'P-JADVAL (SHEETS) + lokatsiya-bog'lash + batching + qaytgan mijoz
- **Sheets tizimi (4 scope):** services / income / expense / categories — har sahifada Google Sheets uslubidagi tab'lar. `services/sheetService.js` RAW `sheets` kolleksiyasi bilan ishlaydi (mongoose modelsiz — model hook'lari ichidan har qanday ALS kontekstda xavfsiz). Har scope'da BITTA faol jadval; yangi qator model `pre('validate')` hook'ida faol jadvalga shtamplanadi (Service, Transaction type bo'yicha, 3 kategoriya modeli), `post('save')` da `maybeArchiveFullSheet` — **30 qator to'lsa AVTO-ARXIV** + yangi "Jadval N" ochiladi + egaga bot xabari. `POST/GET/PATCH /api/sheets` — cheklovsiz yangi jadval (nom bilan; yangisi faol, eskisi arxivga), rename. **Arxiv jadval to'liq tahrirlanadi** (qator endpointlari sheet holatiga qaramaydi). **QIDIRUV/HISOBOT CHEGARASIZ:** hech bir mavjud so'rov sheetId bilan filtrlanmaydi — balans, bugungi xizmatlar, eng yaqin mijoz, PDF/Excel barcha jadvallarni qamraydi; tab faqat Mini App ko'rinishini filtrlaydi (mijoz tomonda).
- **Migratsiya `db/migrateSheets.js`** (bayroq `sheets_v1`, idempotent): sheetId'siz qatorlar har user+scope uchun createdAt tartibida 30 talik bo'laklarga bo'linadi ("Jadval 1..N"); oxirgi to'lmagan bo'lak faol. Sinov: 65 qator → 30+30+5 (6/6 PASS).
- **Mini App:** `components/SheetTabs.jsx` (tab bar + ➕ yangi jadval modal + ✏️ rename; arxiv 🗄), Services/Finance(Kirim+Chiqim alohida)/Categories'ga ulandi; draft (yangi qator) faqat FAOL tab'da; sheetId'siz qatorlar faol tab'da ko'rinadi (`rowMatchesSheet`). Categories overview qatorlariga sheetId qo'shildi (saqlangan kategoriya hujjatlaridan; maxsus/default qatorlar faol tab'da).
- **LOKATSIYA → MAVJUD QATOR (`bot/locationBind.js`):** pin faol kirish oqimidan tashqarida kelsa — nom tasdiqlagach bot "Bu manzil qaysi xizmatga tegishli?" so'raydi (LOCATION_BIND holati; [🆕 Yangi xizmat][Bekor]). Javob matn/ovoz: ism/telefon/manzil bo'lagi (`searchService.findServicesByIdentifier`, kelishik-strip bilan) yoki QATOR RAQAMI (`sheetService.findServiceByRowNumber` — faol jadval, createdAt tartibi). Ko'p moslik → `locbind_pick_<i>` tugmalari (nomzodlar conversation'da). Bog'lash: `serviceService.attachLocationToService` — qatordagi manzil NOMI bo'lsa saqlanadi, bo'lmasa pin nomi; mapUrl = `yandexMapsUrl(lat,lng)`; bot javobida manzil-nomli URL TUGMA (Yandex Maps). Legacy LOCATION_QUESTION holatida ha/yo'q bo'lmagan javob ham bind identifikatori deb qaraladi.
- **Mini App→bot xabarnoma BATCHING:** `miniAppNotifyService.send` endi darhol yubormaydi — foydalanuvchi bo'yicha bufer, 2 daqiqa oyna (yoki 25 ta bo'lsa darhol), bitta "📱 Mini App'da so'nggi daqiqalarda N ta o'zgarish" xabari (>3500 belgi bo'lsa bo'laklab). Bulk-delete darhol (sendNow). SIGTERM'da `flushMiniAppNotifications()` (index.js gracefulShutdown).
- **QAYTGAN MIJOZ TAKLIFI:** SERVICE_ENTRY'da telefon mavjud qatorga mos kelsa (hamkor bo'lmasa) — `maybeOfferReturningClient` oxirgi ism/manzil/narxni taklif qiladi (RETURNING_CONFIRM holati, [✅ Ha][✏️ Yo'q] `ret_use/ret_skip` + matnli ha/yo'q); `resumeReturningEntry` tasdiqda yetishmagan maydonlarni to'ldirib entry oqimini davom ettiradi. `_returningChecked` bayrog'i takror so'rashni oldini oladi; multi-entry/OCR yo'llariga tegilmagan.
- **Testlar:** write-read 81/81 (yangi 8.8 SHEETS bo'limi: avto-arxiv, chegarasiz qidiruv, create/rename, qator-raqami, lokatsiya-bog'lash nom saqlash), small-talk 67/67, sheets-migratsiya 6/6, miniapp build OK.

## 2026-07-17 MIJOZLAR BO'LIMI BEKOR — hammasi Xizmatlar jadvalida
- **Client kolleksiyasi BUTUNLAY olib tashlandi:** `models/Client.js`, `services/clientService.js`, `routes/clients.js`, Mini App `Clients.jsx` + nav tabi o'chirildi. Mijoz ma'lumoti (ism/tel/manzil) endi FAQAT Service qatorining o'zida (`clientName`/`clientPhone`/`location`); `Service.clientId` sxemadan olib tashlandi, yangi `Service.isPartner` boolean qo'shildi. Mijoz identifikatsiyasi = TELEFON (bo'lsa), aks holda ism (kichik harfda) — `partnerService.CLIENT_KEY_EXPR` aggregatsiya ifodasi yagona manba.
- **Migratsiya `db/migrateClientsIntoServices.js`** (startup, idempotent, `migrations` kolleksiyasidagi `clients_into_services_v1` bayrog'i bilan): (1) clientId bo'yicha bo'sh ism/tel backfill; (2) hamkor mijoz qatorlariga isPartner belgisi; (3) xizmatsiz mijozga "profil" qatori (sanasiz kutilmoqda, balansga ta'sirsiz; ortiqcha manzillar izohda); (4) hamkor standarti eng oxirgi qatorda aks etmagan bo'lsa shartnoma qatori; (5) har aktiv mijoz uchun qator borligi TEKSHIRILADI — muammo bo'lsa bayroq yozilmaydi (qayta uriniladi). `clients` kolleksiyasi ZAXIRA sifatida qoladi (kod o'qimaydi). In-memory sinov: 12/12 PASS (backfill, profil, hamkor standarti, idempotentlik).
- **YANGI HOLAT `bajarilmadi` (SERVICE_STATUS.NOT_DONE):** vaqti kelib amalga oshmagan ish (mas. mashina buzildi) — bekor EMAS: balansga yozilmaydi (bajarilgan bo'lsa daromad qaytariladi), sanasi keyin tahrirlanib qayta rejalashtirilishi mumkin. `markServiceNotDone`/`reopenService`/`setServiceStatus` (yangi `PATCH /services/:id/status` — 4 holatning istalgan biriga, shu jumladan orqaga `kutilmoqda`ga). `completeService` endi istalgan holatdan ishlaydi; repair endi NOT_DONE+CANCELLED (NO_INCOME_STATUSES)da qolib ketgan daromadni qaytaradi. Bot: confirm klaviaturasida "⚠️ Bajarilmadi" tugmasi (`notdone_<id>`), matnda "bormadim/qilmadim/yo'q" → not_done (bekor endi faqat "bekor/kerak emas"); STATUS_UPDATE `newStatus` enum'ida `bajarilmadi` (gemini.js/prompts.js).
- **Xizmatlar jadvali ustunlari (spec bo'yicha AYNAN):** Ism | Tel | Sana | Manzil | Narx | To'lov turi | Izoh | Holat. Holat dropdown 4 qiymat, hech qanday o'tish cheklovi yo'q. mapUrl va To'lov holati tahriri ℹ️ tafsilot modalida (funksiya yo'qolmadi). Har maydon IXTIYORIY bo'lib qoldi — bo'sh katak xato emas, istalgan vaqt tahrirlanadi (draft: ism YOKI tel yetadi).
- **Hamkorlik endi "eng oxirgi qator" falsafasida:** standart narx/manzil = o'sha telefon/ismga tegishli ENG OXIRGI (createdAt bo'yicha) qatordan. `upsertPartnerContract` YANGI shartnoma qatorini yaratadi (sanasiz kutilmoqda, isPartner, notes 'Hamkorlik shartnomasi') va mavjud qatorlarni isPartner deb belgilaydi; alohida standart saqlash/sinxronlash YO'Q — createService hamkor tashrifida narx/manzilni o'sha qatordan meros oladi (server tomonda, barcha kirish yo'llari uchun) va yangi qator o'zi keyingi standart bo'ladi. Post-save "Bekor qilish" shartnoma qatorini o'chiradi — eski standart o'z-o'zidan qaytadi (`saved.type='partner'` endi `serviceId`).
- **Mijoz-funksiyalari telefon bo'yicha guruhlab:** "eng ko'p to'lagan/chaqirgan" (`/analytics/clients`, reportInsights.topClient), hisobot Mijozlar bo'limi/Excel varag'i (`clientGroupRows` — ism/tel/manzillar/xizmat soni/to'langan/qarz), hamkor hisoboti (`getPartnerReportRows`). CLIENT_EDIT (`updateClientInfo`) ism/telni BARCHA mos qatorlarda yangilaydi; PAYMENT_UPDATE (`recordServicePayment`) identifikatsiya (phone/name) bo'yicha; disambiguatsiya nomzodlari `{name, phone}` bo'lib conversation'da (`pick_client_<idx>` endi indeks).
- **Qidiruv FAQAT Xizmatlar jadvalida:** `listServices`/`searchServices` ism/tel/manzil/izoh + raqamli so'rovda narx (summa). Bosh sahifa qidiruvi endi `/services?search=` (natija — xizmat qatorlari, ServiceDetailModal ochiladi); agent `search_data` endi clients qaytarmaydi.
- **O'chirish soddalashdi:** bulk 'clients' = 'services'; `restoreClientWithServices`/client restore UI olib tashlandi; listDeleted endi services/transactions/items/reminders.
- **i18n uz+ru:** status.bajarilmadi, home.addServiceBig/searchPlaceholder, ui.rowsCount; nav.clients va clients.* bloklari o'chirildi. CSS: `.badge-notdone`.
- **Testlar:** `test:write-read` 68/68 (yangi: bajarilmadi o'tishlari daromad bilan, hamkor merosi, updateClientInfo, summa-qidiruv, telefon bo'yicha to'lov), `test:small-talk` 67/67, miniapp build OK, migratsiya sinovi 12/12.

## 2026-07-14 (4) Bosh sahifa — dashboard (jadval EMAS)
- **/stats/home kengaytirildi** (bitta so'rovda hammasi): `balance` (getSummary('all') — barcha vaqt), `nextClient` (mavjud getNextClient — bugungi kutilayotganlardan vaqtga eng yaqini), `unpaidFines {count,total}` (fine reminder'lar: transactionId=null, cancelled emas — getFineStats.unpaidCount mezoni), avvalgi todayServices/monthSummary saqlangan. Yangi hisob mantiq ixtiro qilinmagan.
- **Home.jsx tartibi (spec bo'yicha):** jonli sana+vaqt (useNow, 30s interval) + CBU kursi chipi → katta umumiy balans (balance-hero) → "Hozir kimga borish kerak?" kartasi (so'rovsiz; bo'lmasa "Bugun uchun barcha ishlar tugadi 🎉"; bosilsa ServiceDetailModal) → to'lanmagan jarima ogohlantirishi (`.fine-alert`, faqat count>0 da, bosilsa Eslatmalar tabiga) → mijoz qidiruvi → bugungi BARCHA xizmatlar (har kartada rangli holat yorlig'i: Kutilmoqda/Bajarildi/Bekor qilindi; pending'da tezkor ✓) → shu oy Kirim/Chiqim alohida (io-row).
- **Qidiruv summa bo'yicha ham:** `listClients` raqamli so'rovda ("150000"/"150 000") shu narxdagi xizmati bor mijozlarni ham topadi (Service.distinct clientId, price=N) — natija tap → mijoz tafsiloti (mavjud oqim).

## 2026-07-14 (3) Kategoriya dublikatlari: aqlli moslashtirish + birlashtirish vositasi
- **Yagona variant-matcher:** `utils/nameVariant.js` (`isSpellingVariant`/`findVariantMatch`) — apostrof/registr farqi, qo'sh yoki tushib qolgan harf ("plasmasa"~"Plassmassa"), ko'plik "-lar", 1-2 harflik xato (uzunlikka bog'liq Levenshtein chegarasi; ≤4 harf faqat aynan, so'z soni farqli nomlar hech qachon variant emas — "Yengil temir" vs "Og'ir temir" alohida qoladi).
- **Kiritish payti (TALAB 1):** `ensureExpenseCategory`/`ensureIncomeCategory`/`ensureMaterialCategory` va `resolveMaterialName` yangi kategoriya ochishdan OLDIN variant tekshiradi — mos topilsa MAVJUD nomga yozadi, dublikat yaratmaydi. Qo'shimcha: `classifyIntent` endi mavjud kategoriya ro'yxatlarini (60 tagacha/tur) prompt'ga qo'shadi (`loadKnownCategoriesSafe` — dynamic import, xatoda null/fail-safe; qo'shimcha Gemini chaqiruvi YO'Q) — Gemini imlo variantini ro'yxatdagi aynan yozilish bilan qaytaradi.
- **Birlashtirish vositasi (TALAB 2):** Kategoriyalar sahifasida "🔀 Dublikatlarni birlashtirish". `GET /categories/duplicates` — deterministik juftlar + `suggestDuplicatePairs` (bitta Gemini chaqiruvi, xatoda deterministik natija qoladi); har juftda a/b {name,value,count,isDefault} + suggested (default tomon yoki ko'proq ishlatilgani). `POST /categories/merge` (requireDeleteCode 1990) — tranzaksiyalar qoladigan nomga ko'chiriladi (material: barcha imlo-variant materialName'lar birga, auto-izoh qayta quriladi, foydalanuvchi izohi tegilmaydi), yutqazgan saqlangan kategoriya soft-delete, bot xabar oladi. HIMOYA: default/tizim kategoriyani o'chirish taqiqlangan (default faqat qoladigan bo'la oladi — UI'da qulf, backendda xato); avtomatik birlashtirish yo'q — har juftlik foydalanuvchi tasdig'i bilan.

## 2026-07-14 (2) Jadval sahifalariga xos talablar (filtr/jami/statistika)
- **Xizmatlar:** status segmentlari o'rniga FUNNEL filtr — Barchasi (standart) / Bugungi / Kelajakdagi / Tarixdagi / Oy bo'yicha (ikkinchi select'da oxirgi 12 oy). Filtrlash mijoz tomonda (hamma xizmat bir marta yuklanadi); jadval tepasida FAQAT ko'rinayotgan xizmatlarning jami narxi (boshqa kirim turlari aralashmaydi). Sanasi yo'q xizmat faqat "Barchasi"da.
- **Moliya Kirim:** barcha kirimlar BITTA jadvalda, "Turi" ustuni (xizmat/material/buyum/qo'lda kirim/hamkorlik). Hamkorlik tashrifi tx.serviceId → service.clientId → client.isPartner zanjiri bilan aniqlanadi (Finance load'ida /services + /clients ham olinadi). Turi filtri + davr segmentiga "Barchasi" qo'shildi; jami summa filtr bo'yicha.
- **Moliya Chiqim:** kategoriya filtri (variantlar yuklangan yozuvlardagi mavjud kategoriyalardan, jarima='Moshina jarimasi' yorlig'i bilan) + jami summa.
- **Kategoriyalar:** material qatorlarida "Miqdori (kg)" (overview.totalKg) va Jami ustunlari; qator bosilganda (onRowOpen) yozuvlar sahifasi ochiladi.
- **Mijozlar:** ustunlar — ism/telefon/manzil(lar birlashtirilib, tahrir birinchisini yangilaydi)/xizmatlar soni/to'langan summa/qarz/hamkorlik/standart narx. `listClients` endi har mijozga servicesCount/totalPaid/currentDebt qo'shadi (bitta Service.aggregate; qarz = FAQAT bajarilgan xizmatning to'lanmagan qismi). Qator (o'qish-uchun katak/raqam) bosilganda chuqur tafsilot modali — o'zgarishsiz.
- **SheetTable yangi prop:** `onRowOpen` — o'qish-uchun katak/qator raqami bosilganda; tahrir katakları avvalgidek editor ochadi.

## 2026-07-14 Umumiy jadval (spreadsheet) ko'rinishi — Xizmatlar/Moliya/Kategoriyalar/Mijozlar/Buyumlar
- **Yangi umumiy komponent `miniapp/src/components/SheetTable.jsx`:** Google Sheets uslubi — qator raqamlari, ustun sarlavhalari, katakni bosib joyida tahrirlash (matn/raqam/sana/datetime/select), enum ustunlarda DROPDOWN, ustun kengligini sudrab o'zgartirish (localStorage `sheet.widths.<id>` da saqlanadi), tepada BARCHA ustunlar bo'yicha qidiruv, "+" bilan jadval oxiriga bo'sh qator (alohida forma YO'Q), birinchi ustun + qator raqami sticky (gorizontal skrollda muzlatilgan), qator ostidan yoyiladigan tafsilot (`rowDetail` — ovoz/asl matn).
- **Biznes mantiq chetlab O'TILMAYDI:** har katak tahriri sahifa bergan `apply` orqali MAVJUD endpointlarga boradi: xizmat narxi → `PUT /services/:id` (bajarilganda daromad qayta hisoblanadi), Holat dropdown → `complete`/`cancel` (daromad yoziladi/qaytariladi; o'tishlar holatga qarab cheklanadi — orqaga qaytarish yo'q), To'lov holati → `paidAmount` orqali (qisman — `AmountPromptModal`), tranzaksiya → `PUT /transactions/:id` (xizmatga bog'langan daromad summasi backend xabari bilan rad etiladi), buyum Holat: Mavjud→Sotilgan (SellModal, kirim yoziladi) / Berib yuborilgan (GiveModal). Bot xabarnomalari (notifyMiniApp*) server tomonda — jadval tahriri ham xabar yuboradi.
- **Draft (yangi qator) immediate-save falsafasida:** har maydon ixtiyoriy; minimal identifikatsiya to'planganda avtomatik POST (xizmat: ism YOKI telefon; mijoz: ism [+telefon oddiyda]; tranzaksiya: summa/kategoriya/izohdan biri; kategoriya/buyum: nom). Saqlangunga qadar qiymatlar lokal, ✕ bilan bekor.
- **Sahifalar:** Services (segment: Bugun/Kutilmoqda/Bajarildi/Barchasi), Finance (balans/grafik kartalari qoldi; Kirim va Chiqim ALOHIDA jadvallar — tur jadval bilan belgilanadi, POST'da type shundan), Categories (bitta jadvalda Kerakli buyumlar/materiallar/kirim/chiqim/Boshqa qatorlari, › bilan yozuvlar jadvaliga; yozuvlar jadvalida sana/summa/izoh, materialda kg va 1kg narxi tahrirlanadi), Clients (hamkor select, standart narx faqat hamkorda), Items (holat dropdown; buyum maydonlarini tahrirlash API'si yo'q — mavjud qatorlar read-only). Sozlamalar/Eslatmalar/Hisobotlar TEGILMADI.
- **O'chirish:** har jadvalda 🗑 → mavjud `ConfirmDeleteModal` (1990-kod) — o'zgarmagan.
- **CSS TRAP:** `.sheet-table thead th { position: relative }` (resize-handle uchun) sticky ustun sarlavhasini yengardi — `thead th.sheet-pin/.sheet-rownum` aniqroq selektor bilan tuzatilgan. Resize persist: pointerup React renderdan oldin — oxirgi kenglik lokal o'zgaruvchidan yoziladi.
- **i18n:** yangi `sheet.*` kalitlari uz+ru'da; `items.status.*` yorliqlari spec bo'yicha (Mavjud/Sotilgan/Berib yuborilgan).

## 2026-07-06 To'liq audit tuzatishlari (zombi daromad, bulk qamrov, AI panel, va h.k.)
- **🔴 Zombi daromad yopildi:** `Service.incomeManuallyRemoved` yangi bayroq. Income tranzaksiya qasddan o'chirilganda (bitta yoki bulk) xizmatga belgi qo'yiladi; `ensureServiceIncome`/repair bayroq turganda daromadni HECH QACHON qayta yaratmaydi (avval `purgeOld` soft-deleted tx'ni butunlay o'chirgach, repair uni qayta tiriltirardi). Bayroq faol income topilganda, tiklashda (`relinkRestoredTransaction`, `restoreServiceLinks`) va yangi bajarilishda avtomatik tozalanadi.
- **Mini App AI panel OLIB TASHLANDI:** `routes/ai.js` + `Home.jsx AiChatPanel/FloatingAiButton` + `api.streamPost` o'chirildi — panel `mode='query'`da guard'siz STATUS_UPDATE/PAYMENT_UPDATE yozuvlarini bajara olardi. Tabiiy-til muloqot faqat botda; deterministik mijoz qidiruvi joyida.
- **Bulk delete qamrovi:** `services` target endi faqat Service + serviceId'ga bog'liq tranzaksiyalar (qarz/jarima/material/buyumga TEGMAYDI); `all` targetga Reminder qo'shildi; tranzaksiyalar qamrovga kirganda DONE xizmatlarga `incomeManuallyRemoved` qo'yiladi. `purgeOld` endi Reminder/DebtPayment/3 kategoriya modelini ham tozalaydi; `listDeleted`/`restoreByIds` reminders'ni qamraydi.
- **Hisobot insights null-sana:** `$year/$dayOfWeek` endi `$cond`+`$type` bilan — sanasi yo'q xizmat "hammasi" davri PDF/Excel'ni yiqitmaydi, "Sana kiritilmagan / Дата не указана" guruhi bo'lib ko'rinadi.
- **editSavedEntry valyuta:** `editedKey` endi `editFieldToEntry` orqali ("money"≠"amount" bug) — editField+newValue yo'lidagi summa tahriri ham USD→so'm konvertatsiyadan o'tadi; `applySavedEntryUpdate` summa bilan birga originalAmount/originalCurrency/exchangeRateUsed metasini DB'ga sinxronlaydi (updateTransaction/editService qabul qiladi).
- **skipBalance endi Reminder'ning O'ZIDA** (`Reminder.skipBalance`): updateDebtReminder "transactionId yo'q" dan xulosa chiqarmaydi — skipBalance qarzning istalgan tahriri balans tranzaksiyasini xato yaratmaydi.
- **AUTH_DEV_BYPASS production'da QATTIQ o'chiq** (env.js + auth.js isProd defense-in-depth) + getEnvIssues ogohlantirishi.
- **cancelService/reverseIncome:** link uzilgan bo'lsa ham serviceId bo'yicha faol income'lar qaytariladi; repair CANCELLED xizmatlarda qolib ketgan faol daromadlarni ham soft-delete qiladi.
- **Kichiklar:** i18n uz/ru to'liq parity (22 kalit ru'ga qo'shildi, 5 o'lik ru kalit + o'lik AI kalitlari o'chirildi); `findServiceByIdentifier` faqat ISO ko'rinishni sana deb oladi; `parseHumanDateTime` "N soat/kun oldin"ni O'TMISHga hisoblaydi; `applyRawValue('serviceDateTime')` parseUzbekDate'ni ham ishlatadi ("30-iyun" qabul); `getFineStats/getMonthlyFineRows` faqat jarima yozuviga bog'langan to'lovlarni sanaydi (double-count yo'q); universal "bekor" saqlangan yozuvni faqat 10 daqiqa ichida o'chiradi (`collected.savedAt`); `mapClientRows` N+1 o'rniga bitta aggregatsiya.
- **Self-check:** `npm run test:write-read` va `npm run test:small-talk` PASS; miniapp build OK.
...
- **Saqlash/normalizatsiya:** `flow.js`da `normalizeIncomeCategory` va umumiy category merge guard; `financeService.normalizeCategory` income'da dinamik nomni saqlaydi va create/update'da `ensureIncomeCategory` chaqiradi. Expense fallback endi faqat keyword emas: `magazinga ishlatdim` kabi aniq otlardan `Magazin` dynamic kategoriya yasaydi; haqiqatan noaniq "pul berdim/nimadir" holati `boshqa_chiqim`.
- **AI:** Gemini schema/prompt `INCOME_ENTRY.category`ni ham chiqaradi ("ijaradan 1 mln tushdi" -> category `ijara`); multi-entry income bo'laklarida category saqlanadi. Agent post-save edit/multi summary/new-vs-edit qoidalari income categoryni hisobga oladi; bot summary/fallback javoblarida income category ko'rsatiladi.
- **Mini App:** Finance modal income va expense uchun category yuboradi; dropdown tanilgan + dinamik kategoriyalarni ko'rsatadi va "Yangi kategoriya" inputi backend auto-create bilan ishlaydi. Categories sahifasida "Kirim kategoriyalari" bo'limi + detail yozuvlari qo'shildi. i18n: `categories.incomes/customCategory/customPlaceholder`, `category.svalka` uz/ru.
- **Tekshiruv:** backend node --check (o'zgargan backend fayllar) OK; Mini App `npm run build` OK; `npm run test:write-read --workspace musir-yoq-backend` 38/38 PASS (dynamic income `Ijara`, default `svalka`, fallback `Magazin` qamraldi); `git diff --check` OK (faqat CRLF warninglari).

## 2026-07-05 MULTI-ENTRY: bitta xabarda bir nechta yozuv (aralash turlar) + to'plam tahrir/bekor
- **BUG:** "ovqatga 60 ming, benzinga 100 ming ketdi" — Gemini classify BITTA obyekt qaytarardi, faqat birinchi yozuv saqlanardi (ma'lumot yo'qotish). Endi classify sxemasida `fields.entries[]` (10 tagacha): har bo'lak o'z `kind`i bilan — expense | income | service | material_sale | item_sale | item_giveaway (ARALASH bo'lishi mumkin: "Sardorga bordim 200 oldim, benzinga 50 ketdi" → service + expense). Prompt MULTI-ENTRY bo'limi + 3 misol; subIntent birinchi bo'lakdan, flat fields = birinchi bo'lak (moslik).
- **Normalize (gemini.js):** `MULTI_ENTRY_KIND_TO_INTENT`/`MULTI_ENTRY_INTENTS` eksport; `normalizeMultiEntries` har bo'lakni O'Z intent normalizatsiyasidan o'tkazadi (`normalizeExtractedFieldsByIntent`), identifikatsiyasiz bo'lak tashlanadi (service: ism/tel, material: nom, item: nom, expense/income: summa/izoh/toifa); <2 valid bo'lsa entries yo'q (yakka oqim).
- **Agent (`handleMultiEntry`):** runAgent ENG BOSHIDA (`resolveMultiEntries`: conf≥0.7, action multi-qamrovda, entries≥2; pending sessiya reset). 2 bosqich: (1) TAYYORLASH — har bo'lak yakka oqim bilan bir xil: applyDateTimeCorrection → attachEntrySource (ovoz) → applyEntryDefaults → applyPartnerVisitDefaults → USD bo'lsa applyCurrencyConversion (kurs yo'q — HECH NARSA saqlanmay so'mda so'raladi); (2) SAQLASH — `executeToolFlow` (deterministik, conversation:null). Buyum nomi noaniq (needsConfirmation) bo'lak — saqlanmaydi, xulosada "alohida kiriting". Identifikatsiyasi yetmagan bo'lak boshqalarni BLOKLAMAYDI: saqlangach birinchi chala bo'lak uchun `startEntry` slot-filling boshlanadi (xulosa + savol birga).
- **Post-save to'plam:** ENTRY_SAVED, `savedIntent:'MULTI_ENTRY'`, `saved:{type:'multi', refs:[savedRefFromResult...]}`, `entries` meta (kind/intent/ref/amount/category/name). Xulosa `ui.multiSavedSummaryText` (keycap raqamlar, tur emojilari, jami kirim/chiqim — xizmat puli jamiga kirmaydi). ❌ tugma/"bekor" — HAMMASI (`undoSavedEntry case 'multi'` har ref'ni o'z turi bo'yicha qaytaradi); "2-sini bekor qil"/"benzinni o'chir" — `handleMultiSavedText` (message.js routeSavedEntry boshida) faqat o'shani o'chiradi; ✏️/matn tahrir — `editMultiSavedEntry`: `findMultiEntryTarget` (tartib raqami "2-sini/2-yozuvni/ikkinchisini", toifa, nom, izoh; faqat YAGONA moslik) + `applySavedEntryUpdate` intent bo'yicha (tx/service/item). `classifyPostSaveMessage` MULTI_ENTRY'da sameIntent = multi-qamrovdagi barcha turlar.
- **Tekshiruv:** 30/30 sof mantiq testi — goal talab qilgan 3 stsenariy (2 chiqim; xizmat+chiqim; 3+ aralash) + tasnif + target topish + xulosa; node --check 5 fayl OK. Jonli Gemini testi deploy'dan keyin.
...
## Current state
- 2026-07-05: YOZISH ISHONCHLILIGI AUDITI (barcha kiritish yo'llari) — "ba'zi yozuvlar tasodifiy yo'qoladi" shikoyati bo'yicha 9 ta zaif joy tuzatildi. BOT: (1) `index.js` `drop_pending_updates` olib tashlandi (webhook+polling) — avval HAR deploy/restart oynasida yozilgan xabarlar Telegram navbatidan jimgina tashlanardi (asosiy yo'qolish sababi); (2) SIGTERM endi darhol `process.exit` emas — 8s yumshoq to'xtash (yarim yozilgan amallar tugaydi); (3) `bot.js` per-user KETMA-KETLIK navbati (webhookda bir userning parallel xabarlari Conversation/session holatini poygada buzardi; media_group istisno — settle-timer bilan deadlock bo'lmasin); (4) `bot.catch` endi stack loglaydi + egaga aniq xabar yuboradi (avval jim); (5) `message:location` handler try/catch bilan himoyalandi; (6) `agent.js executeToolFlow` tool xatosini serverda stack bilan loglaydi; (7) `enterPostSaveState`/multi-entry post-save: yozuv SAQLANGACH holat-saqlash yiqilsa "xatolik" demaydi (yozuv bazada!) — xulosa tugmasiz qaytadi, sabab logda; (8) `getConversation`/`getOrCreateConversation` atomar upsert (findOne+create dublikat-kalit poygasi tuzatildi); (9) API error handler 5xx da stack + metod/yo'l loglaydi. MINI APP: barcha ro'yxat sahifalari (Home/Finance/Services/Clients/Items/Reminders/Categories) avval yuklash xatosida JIM bo'sh ro'yxat ko'rsatardi (foydalanuvchi "yozuvim saqlanmabdi" deb o'ylardi) — endi `components/LoadError.jsx` banner ("yozuvlaringiz bazada turibdi") + Qayta urinish; i18n `common.loadError/retry` uz+ru. TEST: `backend/scripts/write-read-test.mjs` (`npm run test:write-read`, mongodb-memory-server devDep, in-memory Mongo — production bazaga tegmaydi) — har kiritish turi (xizmat kelajak/tarixiy, kirim, chiqim, shtraf=dinamik kategoriya, material, buyum entry/sale/giveaway, qarz+balans tiklash) yozish→o'qish yo'li + xato yo'llari + tenant izolyatsiyasi; 34/34 o'tdi. "Saqlandi" xabarlari DOIM DB amalidan keyin qurilishi tasdiqlandi (finalizeEntry → executeToolFlow → keyin xulosa). Tekshiruv: `node --check` 5 fayl OK, `vite build` OK.
