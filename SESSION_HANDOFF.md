...
- **Notify service:** `backend/src/services/miniAppNotifyService.js` qo'shildi. Create = faqat requestda kelgan maydonlar; update = faqat diff bo'lgan maydonlar; delete = faqat nom/identifikator; bulk delete = count summary. `notifyOwner` tenant kontekstdan joriy Mini App userga yuboradi.
- **Route qamrovi:** services (post/put/complete/cancel/reschedule/delete), clients (post/put/delete), transactions va finance/transactions (post/put/delete), items (post/sold/give-away/delete), categories material create, reminders (post/done/cancel/snooze/delete), data delete, system bulk-delete/reset. Category route duplicate eski notify bermasligi uchun `createMaterialCategory(..., { notify:false })` ishlatadi.
- **Self-check:** node syntax checks OK; small-talk/classify smoke OK; notification builder smoke OK; backend write/read 41/41 PASS; Mini App build OK.
...
- **(2) Mini App bajarildi → bot xabari:** bu oqim keyin umumiy Mini App CRUD notification helperiga ko'chirildi (`miniAppNotifyService`). `routes/services.js PATCH /:id/complete` endi service old/new diffini yuboradi (faqat o'zgargan maydonlar: holat, to'lov holati, narx va h.k.). `bot/notify.js` (`attachNotifierBot`/`notifyOwner`) hali ham yagona yuborish mexanikasi; Cron "bajarildimi?" so'rovi va vaqt eslatmasi `status=PENDING` filtrida — bajarildi bo'lgach o'z-o'zidan so'ramaydi.
