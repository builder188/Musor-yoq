...
export async function listClients({ search = '' } = {}) {
  ...
  if (search) {
    ...
    filter.$or = [{ name: rx }, { phone: rx }, { location: rx }];
  }
...
// Mijoz tafsiloti: xizmatlar tarixi bilan.
...
export async function listDebtors() {
  return Client.find({ totalDebt: { $gt: 0 }, ...notDeleted })
    .sort({ totalDebt: -1 })
    .lean();
}
